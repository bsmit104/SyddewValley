...
public class InventoryUI : MonoBehaviour
{
    ...
    public GameObject selectedItemDisplayPrefab;
...
void EndDrag(BaseEventData data)
{
    ...
    {
        PointerEventData pointerData = (PointerEventData)data;
        RectTransform slotPanelRectTransform = slotPanel as RectTransform;
        RectTransformUtility.ScreenPointToLocalPointInRectangle(slotPanelRectTransform, pointerData.position, null, out Vector2 localPoint);

        int targetSlotIndex = GetTargetSlotIndex(pointerData.position);

        // Check if dropped into chest inventory UI
        HandleDropIntoChest(pointerData.position);

        // Check if dropped into trash can
...
{
    ChestInventoryUI chestInventoryUI = FindObjectOfType<ChestInventoryUI>();
    if (chestInventoryUI != null)
    {
        ...
        if (chestSlotIndex >= 0)
        {
            ...
            chestInventoryUI.chestInventory.AddItem(draggedItemStack.item, draggedItemStack.stackSize);
    ...
    public void UpdateInventoryUI()
    {
        for (int i = 0; i < slots.Count; i++)
        {
            ...
            {
                Inventory.ItemStack itemStack = inventory.items[i];
...
//works with everything else
    ...
    //         selectedItemDisplay.transform.SetParent(playerHeadTransform);
    //         selectedItemDisplay.transform.localPosition = new Vector3(0, 2, 0);
    //         selectedItemDisplay.transform.localRotation = Quaternion.identity;
    //         selectedItemDisplay.transform.localScale = Vector3.one;
    //     }
    // }
...
//     if (selectedItem != null)
//     {
//         // Instantiate the selected item display prefab
//         selectedItemDisplay = Instantiate(selectedItemDisplayPrefab);
//         Image itemImage = selectedItemDisplay.GetComponent<Image>();
//         itemImage.sprite = selectedItem.itemIcon;

//         // Set the parent to the player head transform
//         selectedItemDisplay.transform.SetParent(playerHeadTransform);

//         // Adjust the local position, rotation, and scale
//         RectTransform rectTransform = selectedItemDisplay.GetComponent<RectTransform>();
//         rectTransform.anchoredPosition = new Vector2(0, 50); // Adjust the height as needed
...
//     int GetTargetSlotIndex(Vector2 position)
//     {
//         for (int i = 0; i < slots.Count; i++)
//         {
//             RectTransform slotRectTransform = slots[i].GetComponent<RectTransform>();
